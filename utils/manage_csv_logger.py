...
def create_csv_logger(csv_directory, wavelet, experiment, dataset_type):
    """
    ...
    csv_logger_path=os.path.join(csv_directory, f"model_{dataset_type}-{wavelet}-{experiment}-log.csv")
    return CSVLogger(csv_logger_path, append=True)

def get_csv_logger(wavelet, experiment, dataset_type):
    """
    ...
    csv_logger=create_csv_logger(csv_directory, wavelet, experiment, dataset_type)
